...
class Application:
    ...
    def app_size_x(self, value=500):
        if value > 20 and value < 10000:
            ...
            raise ApplicationError(
                "Value must be within the range of 20 - 10000.")
    ...
    def _create_page(self, parent=None):
        ...
        try:
            temp = Frame(self._root,
                         width=self._app_size_x,
                         height=self._app_size_y)
    ...
    def _logout(
        self
    ):  # Logs the user out of the program, returning them to login page
        failed = False
        try:
    ...
    def _save_edit_booking(self):
        ...
        try:
            ...
            booking.start.hour = int(
                self._edit_selected_check_in_output_hour.get())
            booking.start.minute = int(
                self._edit_selected_check_in_output_minute.get())
            booking.user.postcode = self._edit_selected_postcode_output.get()
            booking.end.hour = int(
                self._edit_selected_check_out_output_hour.get())
            booking.end.min = int(
                self._edit_selected_check_out_output_minute.get())
            ...
            booking.user.pets = int(
                self._edit_selected_pet_amount_output.get())
    ...
    # to the database
    def _book_stay(self):
        time_change = management.timedelta(weeks=78)
        current_time = management.datetime.now()
        time_cap = time_change + current_time
        time_cap = time_cap.date()

        start_date = self._start_date_entry.get_date()
        start_date_hour = self
        end_date = self._end_date_entry.get_date()
        if start_date > time_cap:
            box.showerror("Booking Error",
                          "You cannot book over 18 months in advance.")
            return
        ...
        try:
            ...
            start_date = "{}/{}/{}".format(start_date[8:10], start_date[5:7],
                                           start_date[0:4])

            end_date = str(end_date).replace("-", "/")
            end_date = "{}/{}/{}".format(end_date[8:10], end_date[5:7],
                                         end_date[0:4])
            ...
            end_date = management.FormattedTimeAndDate(end_date, int(18),
                                                       int(0))
        ...
        if found_issue_number:
            ...
            box.showwarning(
                "Phone Number",
                "The phone number provided is {} numbers long it should be 11."
        ...
        try:
            ...
            if e == "too many values to unpack (expected 2)":
                box.showwarning(
                    "Name Error",
                    "Please provided both your first and last name.")
        ...
        if management.BookingManagement.booking_available(
                start_date, end_date):
            booking = management.Booking(start_date,
                                         end_date,
                                         user_data,
                                         create=True)
    ...
    def _new_account_page_submit(self, event=None):
        ...
        if len(new_user_name) >= 16:
            box.showwarning("""Username is too large, please choose a username
                under 16 characters.""")
            return
    ...
    def _change_password(self, event=None):
        account = self._get_account()
        username = account["name"]
        self._username_display.config(text=username)

        if account is None:
            self._go_to_edit_accounts()

        new_password_1 = self._new_pass_entry.get()
        new_password_2 = self._confirm_new_pass_entry.get()

        if new_password_1 != new_password_2:
            box.showwarning("Password Error", "Passwords do not match.")
            return
        ...
        if username == self._user.username:
            ...
            user = management.User.get_user_from_username(username)

        try:
            success = management.UserManager.change_password(
                password=new_password_1, acting_user=self._user, user=user)
        except management.PermissionDenied as r:
            box.showerror("Permission Denied", r)
            return
        except management.PasswordValidationError as e:
            box.showwarning("Password Error", e)
            return

        box.showinfo("Password Changed",
                     f"{user.username} password has been changed.")
        self._username_display.delete(0, tkinter.END)
        self._new_pass_entry.delete(0, tkinter.END)
        self._confirm_new_pass_entry.delete(0, tkinter.END)
    ...
    def _go_to_make_booking(self, event=None):
        default_date = management.datetime.now()
        self._start_date_entry.set_date(default_date)
        self._end_date_entry.set_date(default_date)
        self._full_name_entry.delete(0, tkinter.END)
        self._address_entry.delete(0, tkinter.END)
        self._email_entry.delete(0, tkinter.END)
        self._pet_var.set("0")
    ...
    def _go_to_new_password_page(self, event=None):
        ...
        if account["name"] is None:
            box.showwarning("User Error",
                            "Please select a user to set their password.")
            return

        self._username_display.config(text=account["name"])
    ...
    def _set_edit_selected_with_booking(self, booking=management.Booking):
        ...
        self._edit_selected_check_in_output_minute.insert(
            END, booking.start.minute)
        ...
        self._edit_selected_full_name_output.insert(
            END, "{}".format(booking.user.name))
        self._edit_selected_email_output.insert(
            END, "{}".format(booking.user.email))
        ...
        self._edit_selected_phone_number_output.insert(
            END, booking.user.phone_number)
    ...
    def _edit_selected_booking_delete(self):
        ...
        if booking is None:
            box.showwarning("No Booking Selected",
    ...
    def _check_booking_availability(self, event=None):
        ...
        if booking_available:
            self._booking_availability.config(text="Booking available",
                                              fg="green")
        if not booking_available:
            # self._booking_availability.config(text="Booking unavailable", fg="red")
            box.showwarning("Booking",
                            "The booking requested is currently unavailable.")

        self._booking_availability.after(3000,
                                         self._hide_booking_status_message)
    ...
    def _create_account_action(self):
        ...
        new_user = management.User(new_account_username, new_account_password,
                                   level)
    ...
    def _remove_account_action(self):
        ...
        try:
            result = management.UserManager.remove_user(
                self._user, user_to_remove)
    ...
    def _view_booking_view(self):
        ...
        self._view_selected_check_in_output.config(
            text="{}:{}".format(format_time(booking.start.hour),
                                format_time(booking.start.minute)))
        self._view_selected_check_out_output.config(text="{}:{}".format(
            format_time(booking.end.hour), format_time(booking.end.minute)))
        self._view_selected_full_name_output.config(
            text="{}".format(booking.user.name))
        self._view_selected_email_output.config(
            text="{}".format(booking.user.email))
        ...
        self._view_selected_phone_number_output.config(
            text=booking.user.phone_number)
    ...
    def _open_admin_panel(self):
        print("Username: {} Level: {}".format(self._user.username,
                                              self._user.level))
    ...
    def __init__(self, test_mode=False):
        ...
        self._root.geometry(
            f"{self._app_size_x}x{self._app_size_y}")  # Sets size of window
        ...
        self._username_display = tkinter.Label(
            new_password_page, textvar="")  # Add username variable
        self._username_display.grid(column=0, row=1, pady=(18, 0))

        # Enter new password
        self._new_pass_label = tkinter.Label(new_password_page,
                                             text="Enter new password:")
        self._new_pass_label.grid(column=0, row=2, padx=(0, 180), pady=(30, 0))
        self._new_pass_entry = tkinter.Entry(new_password_page,
                                             width=30,
                                             show="*")
        ...
        self._confirm_new_pass_label.grid(column=0,
                                          ...
                                          padx=(0, 150),
                                          pady=(30, 0))
        self._confirm_new_pass_entry = tkinter.Entry(new_password_page,
                                                     width=30,
                                                     show="*")
        self._confirm_new_pass_entry.grid(column=0, row=5)

        # Confirm Button
        self._confirm_button = tkinter.Button(new_password_page,
                                              text="Confirm",
        ...
        login_page_title = tkinter.Label(login_page,
                                         text="Login Page",
                                         font=25)
        login_page_title.grid(row=1, column=1, pady=(150, 0), padx=(350, 400))
        ...
        self._login_page_ID_text.grid(row=2,
                                      column=1,
                                      pady=(15, 0),
                                      padx=(0, 268))
        ...
        self._login_page_name_entry.grid(row=2,
                                         column=1,
                                         pady=(5, 0),
                                         padx=(0, 0))

        self._login_page_pass_text = tkinter.Label(login_page,
                                                   text="Password:")
        self._login_page_pass_text.grid(row=3, column=1, padx=(0, 280))

        # Password entry for login
        self._login_page_pass_entry = tkinter.Entry(login_page, show="*")
        self._login_page_pass_entry.grid(row=3, column=1, padx=(0, 0))

        # Error message
        self._login_page_error_label = tkinter.Label(login_page,
                                                     text="",
                                                     fg="red")
        self._login_page_error_label.grid(row=5, column=1)

        # Background hide label
        self._background_hide_label = tkinter.Label(login_page, text=" ")
        self._background_hide_label.grid(row=6, column=1, pady=400, padx=400)
        ...
        self._login_page_submit_button.grid(row=4,
                                            column=1,
                                            pady=(15, 0),
                                            padx=(0, 50))
        ...
        self._start_date_label = tkinter.Label(make_booking,
                                               text="Start date:")
        ...
        # Check in time box
        self._check_in_label = tkinter.Label(make_booking,
                                             text="Check in time:")
        # Hour
        check_in_hour = tkinter.StringVar(value="0")

        self._check_in_time_hour = tkinter.Spinbox(make_booking,
                                                   from_=0,
                                                   to=23,
        ...
        self._check_in_time_min = tkinter.Spinbox(make_booking,
                                                  from_=0,
                                                  to=59,
        ...
        # Check out time box
        self._check_out_label = tkinter.Label(make_booking,
                                              text="Check out time:")
        # Hour
        ...
        self._check_out_time_min = tkinter.Spinbox(make_booking,
                                                   from_=0,
                                                   to=59,
        ...
        # Start and end date
        start_date = tkinter.Label(make_booking,
                                   textvar=self._start_date_entry.get())
        start_date.grid(row=4, column=0)  # Start
        end_date = tkinter.Label(make_booking,
                                 textvar=self._end_date_entry.get())
        end_date.grid(row=4, column=1)  # End

        # Check in time
        check_in_time = tkinter.Label(
            make_booking,
            textvar=self._check_in_time_hour.get() +
            self._check_in_time_min.get(),
        )

        # Check out time
        check_in_time = tkinter.Label(make_booking,
                                      textvar=self._check_in_time_hour.get())
        check_out_time = tkinter.Label(make_booking,
                                       textvar=self._check_in_time_min.get())
        ...
        self._pet_var = pet_amount

        # Book Button
        book_button = tkinter.Button(make_booking,
                                     text="Book",
                                     command=self._book_stay,
                                     height=1,
                                     anchor="w")
        ...
        bookings_list = tkinter.Listbox(manage_booking_page,
                                        height=15,
                                        width=50)
        bookings_list.grid(column=0, row=1)
        ...
        admin_true = tkinter.Label(main_menu,
                                   text="Loading",
                                   font=("Helvetica", 12),
                                   fg="green")
        ...
        # Contents of the page
        # Start date
        start_date_label = tkinter.Label(view_selected_booking,
                                         text="Start date:")
        ...
        # Check in time
        check_in_label = tkinter.Label(view_selected_booking,
                                       text="Check in time:")
        ...
        # Check out time
        check_out_label = tkinter.Label(view_selected_booking,
                                        text="Check out time:")
        ...
        # Full name
        full_name_label = tkinter.Label(view_selected_booking,
                                        text="Full name:")
        ...
        # Phone number
        phone_number_label = tkinter.Label(view_selected_booking,
                                           text="Phone number:")
        ...
        email_output = tkinter.Label(view_selected_booking,
                                     textvar="")  # add variable
        email_output.grid(column=1, row=7, padx=(140, 0))
        self._view_selected_email_output = email_output

        # Number of pets
        pet_amount_label = tkinter.Label(view_selected_booking,
                                         text="Number of pets:")
        ...
        price_output = tkinter.Label(view_selected_booking,
                                     textvar="")  # add variable
        ...
        # Contents of the page
        # Start date
        start_date_label = tkinter.Label(edit_selected_booking,
                                         text="Start date:")
        start_date_label.grid(column=0, row=0, padx=(0, 140), pady=(30, 0))
        ...
        # Check in time
        check_in_label = tkinter.Label(edit_selected_booking,
                                       text="Check in time:")
        ...
        # Check out time
        check_out_label = tkinter.Label(edit_selected_booking,
                                        text="Check out time:")
        ...
        # Full name
        full_name_label = tkinter.Label(edit_selected_booking,
                                        text="Full name:")
        full_name_label.grid(column=0, row=4, padx=(0, 140), pady=(30, 0))
        ...
        # Phone number
        phone_number_label = tkinter.Label(edit_selected_booking,
                                           text="Phone number:")
        phone_number_label.grid(column=0, row=6, padx=(0, 140), pady=(30, 0))
        ...
        email_label.grid(column=1, row=6, padx=(140, 0), pady=(30, 0))
        email_output = tkinter.Entry(edit_selected_booking,
                                     textvar="")  # add variable
        email_output.grid(column=1, row=7, padx=(140, 0))
        ...
        pet_amount_label = tkinter.Label(edit_selected_booking,
                                         text="Number of pets:")
        pet_amount_label.grid(column=0, row=8, padx=(0, 140), pady=(30, 0))
        ...
        price_label.grid(column=1, row=8, padx=(140, 0), pady=(30, 0))
        price_output = tkinter.Label(edit_selected_booking,
                                     textvar="")  # add variable
        price_output.grid(column=1, row=9, padx=(140, 0))
        ...
        # Manage accounts
        if not test_mode:
            img_label_manage_accounts = tkinter.Label(manage_accounts_page,
                                                      image=img)
        ...
        guest_accounts_label = tkinter.Label(manage_accounts_page,
                                             text="Guest Accounts")
        ...
        new_account_button = tkinter.Button(manage_accounts_page,
                                            text="New Account",
        ...
        new_password_button.grid(column=1, row=2, padx=(0, 80), pady=(60))
        ...
        self._booking_availability.grid(row=10,
                                        column=1,
                                        columnspan=3,
                                        padx=(0, 340),
                                        pady=(0, 15))
        ...
        new_page_ID_text = tkinter.Label(new_accounts_page,
        ...
        menu_button = tkinter.Button(new_accounts_page,
                                     text="Exit",
                                     command=self._go_to_main_menu,
                                     anchor="w")
